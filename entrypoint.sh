...
# Новые переменные для адресов сервисов
...
# Находим все файлы config.yaml в папке /app/configs и заменяем в них localhost
# sed -i 's/что_менять/на_что_менять/g' /путь/к/файлу
# Мы ищем 'host: localhost' и заменяем на 'host: db' и так далее.
...
# Замена для адресов микросервисов (клиентские подключения в api-gateway)
